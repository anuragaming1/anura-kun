...
class Database {
    constructor() {
        ...
        // Debug: in 5 snippet đầu tiên
        if (this.data.snippets.length > 0) {
            console.log('📋 First 5 snippets:');
            this.data.snippets.slice(0, 5).forEach((s, i) => {
                console.log(`   ${i}. "${s.slug}" (fake: ${s.content_fake?.length || 0} chars)`);
            });
        }
    }
    ...
    async getSnippet(slug) {
        console.log('🔍 [getSnippet] Searching for snippet with slug:', `"${slug}"`);
        ...
        console.log('📊 [getSnippet] Available slugs:', allSlugs.length > 0 ? allSlugs.join(', ') : 'None');
        ...
        if (snippet) {
            console.log('✅ [getSnippet] Snippet found!');
            console.log('📝 [getSnippet] Snippet details:', {
                id: snippet.id,
                slug: snippet.slug,
            ...
            console.log('❌ [getSnippet] Snippet not found');
            ...
            if (similarSlugs.length > 0) {
                console.log('ℹ️  [getSnippet] Similar slugs found:', similarSlugs.map(s => s.slug).join(', '));
            }
