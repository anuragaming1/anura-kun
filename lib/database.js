...
async function getDatabase() {
    if (dbInstance) {
        console.log('Returning existing KV instance');
        return dbInstance;
    }
...
class Database {
    ...
    getSnippetKey(slug) { return `snippet:${slug.toLowerCase()}`; } // Lowercase để case-insensitive
    getAllSlugsKey() { return 'all_slugs'; }

    async createSnippet(slug, content_fake, content_real) {
        const lowerSlug = slug.toLowerCase();
        console.log('Creating snippet:', lowerSlug);

        if (!slug || !content_fake || !content_real) return { success: false, error: 'Missing fields' };

        const exists = await kv.exists(this.getSnippetKey(lowerSlug));
        if (exists) return { success: false, error: 'Slug exists' };

        const secretKey = crypto.randomBytes(32).toString('hex');
        const snippet = {
            slug: lowerSlug,
            original_slug: slug, // Giữ original case nếu cần display
            content_fake,
            content_real,
        ...
        console.log('Snippet created in KV!');
    ...
    async getAllSnippets() {
        ...
        for (const s of slugs) {
            const data = await kv.get(this.getSnippetKey(s));
            if (data) snippets.push(JSON.parse(data));
        }
        return snippets.map(s => s.original_slug || s.slug); // Return list slugs cho available
    }

    async incrementViews(slug) {
        const snippet = await this.getSnippet(slug);
        if (snippet) await kv.set(this.getSnippetKey(slug.toLowerCase()), JSON.stringify(snippet));
    }

    async debugAllSnippets() {
        const all = await kv.smembers(this.getAllSlugsKey()) || [];
        console.log('DEBUG KV: Total snippets:', all.length);
        for (const s of all) {
            const data = await kv.get(this.getSnippetKey(s));
            if (data) {
                const sn = JSON.parse(data);
                console.log(`- \( {sn.original_slug || sn.slug} | fake: \){sn.content_fake.length} | real:\( {sn.content_real.length} | views: \){sn.views}`);
            }
        }
    }

    // Thêm delete/search nếu cần sau, tạm đủ cho create/get/raw
}
...
class Database {
    ...
    async createSnippet(slug, content_fake, content_real) {
        ...
        // Kiểm tra slug đã tồn tại chưa (case-sensitive)
        const existingSnippet = this.data.snippets.find(s => s.slug === slug);
        if (existingSnippet) {
    ...
    async getSnippet(slug) {
        ...
        console.log('📊 Available slugs:', this.data.snippets.map(s => `"${s.slug}"`).join(', '));
        
        // Tìm snippet (case-sensitive)
        const snippet = this.data.snippets.find(s => s.slug === slug);
        ...
        if (snippet) {
            ...
            const caseInsensitive = this.data.snippets.find(s => 
                s.slug.toLowerCase() === slug.toLowerCase()
            );
            
            if (caseInsensitive) {
                console.log('ℹ️  Case-insensitive match found:', caseInsensitive.slug);
                console.log('ℹ️  Requested slug:', slug);
    ...
    async checkSlugAvailable(slug) {
        const exists = this.data.snippets.some(s => s.slug === slug);
    ...
    async deleteSnippet(slug) {
        ...
        this.data.snippets = this.data.snippets.filter(s => s.slug !== slug);
    ...
    async searchSnippets(query) {
        const results = this.data.snippets.filter(s => 
            ...
            (s.content_fake && s.content_fake.includes(query)) ||
            (s.content_real && s.content_real.includes(query))
        );
